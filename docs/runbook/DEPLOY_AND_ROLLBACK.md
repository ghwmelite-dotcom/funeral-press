...
3. **Post-deploy smoke tests deferred to external monitor (2026-05-11).** The in-CI `smoke` job briefly existed (PR #21) but was removed in the same week because Cloudflare's Free-plan edge rejects GitHub Actions runner IPs with HTTP 403 on every `/health` probe — Bot Fight Mode toggled OFF, WAF Custom Rule added, custom `User-Agent` set, all no effect. The remaining culprit is Cloudflare's automatic IP-reputation classification, which can't be bypassed on Free. **Use UptimeRobot (or equivalent) for synthetic monitoring** — its IPs are reputable enough to clear Cloudflare's edge. Add monitors for each `*.funeralpress.org/health` endpoint + the SPA root.
4. ~~No deploy notification~~ — **fixed** 2026-05-08, simplified 2026-05-11. The `notify-deploy` job posts a Slack message on every push-to-main with the commit short-SHA, first line of the message, the actor, and the run URL. Reuses the `SLACK_WEBHOOK_URL` repo secret. No-op when the secret is unset.
