...
export default function OneWeekVersionsDialog({ open, onOpenChange }) {
  const snapshots = useOneWeekStore(s => s.snapshots)
  const restoreSnapshot = useOneWeekStore(s => s.restoreSnapshot)
  const deleteSnapshot = useOneWeekStore(s => s.deleteSnapshot)
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Restore a previous version of your one-week poster.</DialogDescription>
        </DialogHeader>
